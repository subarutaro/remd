...
#if defined(TIP4P) || defined(TIP4PICE) || defined(TIP3P)
#include "4site.cuh"
#elif defined(TIP5P)
#include "5site.cuh"
#endif
...
#pragma omp parallel for num_threads(nthreads)
  for(unsigned int dev=0;dev<ngpus;dev++){
    ...
    default:
      ...
      std::cerr << "error: Bulk and 2D is disabled" << std::endl;
      exit(EXIT_FAILURE);
    }
  }
 ...
 const float wall_length
  ...
  for(unsigned long s=0;s<interval;s++){
    ...
    if(((MODE>>PSHIFT)&MASK) == 3){
      if(((MODE>>CSHIFT)&MASK) == 2){
#ifdef SQUARE
	b.x += t.x*((potvir.x+potvir.y)/A - P*wall_length)*dthalf;
#else
...
#else
    potvir = CalcForcePotVirSwitchingTuned<MODE>
      (r_rep,f_rep,(f3i1*)ar_rep,af_rep,rslt_dev[bid],L,rc02,rswitch,wall_length,nmol,nsite);
#endif
  ...
  if(tid==0){
    ...
    rslt_dev[bid].tra = kin.x + kin.y + kin.z;
    rslt_dev[bid].rot = kin.w;
